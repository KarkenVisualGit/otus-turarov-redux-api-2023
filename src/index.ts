...
export function attachCartEventListeners(localStore: Store) {
  document.querySelectorAll(".remove-from-cart").forEach((button) => {
    button.addEventListener("click", (event) => {
      const target = event.target as HTMLButtonElement;
      const productId = target.dataset.id;
      if (productId) {
        localStore.dispatch(removeFromCart(productId));
      } else {
...
export function renderCart(localStore: Store) {
  const cartElement = document.getElementById("cart");
  if (!cartElement) return;
  cartElement.innerHTML = "";

  localStore.getState().cart.items.forEach((product) => {
...
export function attachEventListeners(localStore: Store) {
